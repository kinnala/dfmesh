...
def test(show=False):
    geo = dfmesh.Rectangle(0.0, 1.0, 0.0, 1.0)

    # p0 = dfmesh.Path([[0.0, 0.0]])
    p1 = dfmesh.Path([[0.4, 0.6], [0.6, 0.4]])

    def edge_size(x):
        return 0.03 + 0.1 * p1.dist(x)

    X, cells = dfmesh.generate(geo, edge_size, show=show, tol=1.0e-10, max_steps=100)
