...
from ._helpers import _dot, _multiply, grp_start_len, unique_rows

__all__ = ["Mesh"]


# TODO def unique_rows + remove import
...
class Mesh:
    ...
    def signed_circumcenter_distances(self):
        if self._signed_circumcenter_distances is None:
            ...
            tmp = np.zeros((self.facets["points"].shape[0],))
            cells = self.cells("facets")
            dists = self.circumcenter_facet_distances
            for i in range(3):
                np.add.at(tmp, cells[:, i], dists[i])
            self._signed_circumcenter_distances = tmp[self.is_interior_facet]
